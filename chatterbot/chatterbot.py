...
from . import utils
from .queues import FixedSizeQueue
...
class ChatBot(object):
    ...
    fixed_queue = FixedSizeQueue()
    reponse_queue = ResponseQueue(fixed_queue)
    ...
    def get_response(self, input_item, session_id=None):
        ...
        # for(input in self.fixed_queue)
        # print(input_item)
        input_statement = self.input.process_input_statement(input_item) 
        # append the queue if any
        self.fixed_queue.append(input_statement)
        ...
        # Check if the input needs more info
    ...
    def followup_question(self, response):
        followup = False
        followup_option = {
            "Sure, what do you need?": True,
            "Let me look into that.. Could you be more specific?": True,
            "What do you mean exactly?": True,
            "Can you elaborate?": True
        }
        
        followup = response in followup_option
        return followup